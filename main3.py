...
for plot in plots:
    ...
    if plot['title'] == 'Gravity-gradient torque vs time':
        ...
        plt.plot(data['t'], tg[:, 0], label='Tₓ')
        plt.plot(data['t'], tg[:, 1], label='T_y')
        plt.plot(data['t'], tg[:, 2], label='T_z')

    elif plot['title'] == 'Total torque vs time':
        tt = np.vstack(data['torque total'])
        plt.plot(data['t'], tt[:, 0], label='Roll')
